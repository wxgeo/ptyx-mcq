...
def test_legacy_correct_minus_incorrect_1():
    ...
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 6
    answers = AnswersData(checked={1, 2}, correct={1}, all={1, 2, 3, 4})
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 0
    answers = AnswersData(checked={1, 2}, correct=set(), all={1, 2, 3, 4})
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == -1
    answers = AnswersData(checked=set(), correct=set(), all={1, 2, 3, 4})
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 6
    answers = AnswersData(checked={1}, correct={1, 2}, all={1, 2, 3, 4})
    # half the score
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 3
    answers = AnswersData(checked={1, 3}, correct={1, 2, 3}, all={1, 2, 3, 4, 5, 6})
    # 2/3 of the score
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 4
    answers = AnswersData(checked={1, 2, 4}, correct={1, 2, 3}, all={1, 2, 3, 4, 5, 6})
    # 1/3 of the score
    assert ScoringImplementations.special_correct_minus_incorrect(answers=answers, score=scores) == 2
...
def test_correct_minus_incorrect():
    ...
    answers = AnswersData(checked={1, 2}, correct={1, 2}, all={1, 2, 3, 4})
    assert ScoringImplementations.correct_minus_incorrect(answers=answers, score=scores) == 6
    answers = AnswersData(checked={3, 4}, correct={1, 2}, all={1, 2, 3, 4})
    assert ScoringImplementations.correct_minus_incorrect(answers=answers, score=scores) == -2
    answers = AnswersData(checked={1}, correct={1, 2}, all={1, 2, 3, 4})
    assert ScoringImplementations.correct_minus_incorrect(answers=answers, score=scores) == 3
    answers = AnswersData(checked={1, 3}, correct={1, 2}, all={1, 2, 3, 4})
    assert ScoringImplementations.correct_minus_incorrect(answers=answers, score=scores) == 0
    answers = AnswersData(checked={1, 3}, correct={1}, all={1, 2, 3, 4})
    assert ScoringImplementations.correct_minus_incorrect(answers=answers, score=scores) == 4
...
def random_evaluation(
    ...
    for i in range(loops):
        ...
        total += getattr(ScoringImplementations, strategy)(answers=answers, score=scores)
