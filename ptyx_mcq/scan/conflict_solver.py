...
class AbstractConflictSolver(ABC):
    """Base class for any conflict solver implementation."""
    ...
    def review_duplicate_pages(self) -> None:
        ...
        for tmp_doc_id, doc_data in self.data_storage.get_all_temporary_ids().items():
            ...
            for page, pic_data in doc_data.pages.items():
                ...
                if same_data or self.select_version(scanned_id, tmp_doc_id, page) == 1:
...
class ConflictSolver(AbstractConflictSolver):
    """Concrete implementation of conflict solver for command line interface."""
